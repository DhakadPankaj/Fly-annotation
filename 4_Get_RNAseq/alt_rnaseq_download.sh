...
# Provide alt_rna_sp.tsv file
sp_list=$1

# Download metadata for Drosophila RNAseq from ENA API
...
# Filter metadata for paired-end reads and save to a new file
cat rnaseq_fly_rawmetadata.tsv | grep "_1.fastq.gz" > Only_paired_splitted_fastq_rnaseqData.tsv
rm rnaseq_fly_rawmetadata.tsv

# Filter species list and remove empty lines
cat $sp_list | cut -f1,2 | awk -F'\t' '$NF ~ /[^ ]/ {print $0}' > tmp.txt

# Filter metadata based on species list and save to a new file
awk -F '\t' 'FILENAME=="tmp.txt"{A[$2]=$2} FILENAME=="Only_paired_splitted_fastq_rnaseqData.tsv"{if(A[$1]){print}}' tmp.txt Only_paired_splitted_fastq_rnaseqData.tsv | cut -f 1,2,5,6,12,13,15,16,18,26,27,28 | sort -n -k 1 > tmp2.txt
...
do
  ...
  if [[ -d ${name} ]]; then
    ...
    grep -iF -h -e "carcass" -e "Testes" -e "testis" -e "ovary" -e "ovaries" -e "body" -e "brain" -e "fat" -e "head" -e "abdomen" -e "embryo" -e "larva" -e "larvae" -e "thorax" -e "adult" -e "salivary" -e "digestive" -e "gut" -e "nervous" $name.txt > tissue_var.txt
    ...
    do
      ...
      if [[ $(grep -i -h -E -w female $i_tmp.txt | grep -v -i -w male | wc -l) -ge "1" ]]; then
        grep -i -h -E -w female $i_tmp.txt | grep -v -i -w male | head -1 >> read_sorted.txt
      else
        cat $i_tmp.txt | head -1 >> read_sorted.txt
      fi
      if [[ $(grep -i -h -E -w male $i_tmp.txt | grep -v -i -w female | wc -l) -ge "1" ]]; then
    ...
    elif [[ $tissue_count -gt 10 ]] && [[ $readsorted -ge 10 ]]; then
      awk '!x[$0]++' read_sorted.txt | head -10 | cut -f 4 > sra.txt
      cat sra.txt | xargs -I sra ~/softwares/enaBrowserTools-0.0.3/python3/./enaDataGet -f fastq --aspera sra
    elif [[ $tissue_count -ge 10 ]] && [[ $readsorted -lt 10 ]]; then
      ...
      do
        ...
        if [[ -f "$i"_1.fastq.gz ]] && [[ -f "$i"_2.fastq.gz ]]; then
          ...
          if [[ $md5_F1 == $ena_md5_F1 ]] && [[ $md5_F2 == $ena_md5_F2 ]]; then
            echo "md5 checked at loop:$loop & it is the same as ENA file"
          else
            rm -r *.fastq.gz logs/
          fi
        fi
